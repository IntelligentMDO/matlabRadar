...
Psi_Total_Divide = {0:30:180};                                              %中心角_Psi
...
for nTheta = 1:length(Theta_Total)
    parfor nPsi = 1:length(Psi_Total)
        ...
        = @(Gamma) interp1(Gamma_dBsm_KDE_Local, p_Gamma_dBsm_KDE_Local, 10*log10(Gamma), 'spline',0)...
                   * 10./(Gamma*log(10));

        IntRange_Gamma_Forward_Total{nTheta,nPsi}...
        = [max(10^(Gamma_dBsm_KDE_Local(1)/10),0), 10^(Gamma_dBsm_KDE_Local(end)/10)];


        F_p_Gamma_Backward_Total{nTheta,nPsi}...
        = @(Gamma, SNR) interp1(Gamma_dBsm_KDE_Local - SNR_Local + SNR, p_Gamma_dBsm_KDE_Local, 10*log10(Gamma), 'spline',0)...
                        * 10./(Gamma*log(10));

        IntRange_Gamma_Backward_Total{nTheta,nPsi}...
        = @(SNR) [max(10^((Gamma_dBsm_KDE_Local(1) - SNR_Local + SNR)/10),0),...
                  10^((Gamma_dBsm_KDE_Local(end) - SNR_Local + SNR)/10)];
    end
end
